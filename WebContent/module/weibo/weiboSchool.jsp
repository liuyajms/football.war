...
<div class="panel panel-default">
	<div class="panel-heading">
		<h3 class="panel-title">
			<fmt:message key="weibo.school" bundle="${weibo}" />
		</h3>
	</div>
	...
	<table id="table" class="table table-striped table-bordered" cellspacing="0" width="100%">
		<thead >
			<tr>
				...
				<th style="width:80px;"><fmt:message key="weibo.viewComment" bundle="${weibo}" /></th>
				<th name="description"><fmt:message key="weibo.content" bundle="${weibo}" /></th>
			</tr>
		</thead>
		<tbody />
	</table>
...
<script type="text/javascript">
	...
	function select(page) {
		restSelect(url + "/school", {"keyword" : $("#keyword").val(), "page" : page, "rows" : rows}, function(json) {
			refreshTable($("#table"), json, [{name: '<fmt:message key="update" />', func: "update"}, {name: '<fmt:message key="weibo.viewComment"  bundle="${weibo}" />', func: "viewComment"}]);
		});
	}
	
	function viewComment(id){
		//menu("weiboComment.jsp?id=" + id + "&w=1");
