...
<body>
	<div style="border:1px solid #999;padding:3px;background:#F0E68C;">
		<div class="panel-heading">
			<h3 class="panel-title">
				<fmt:message key="stats.school.title" />
			</h3>
		</div>
		<div class="col-md-4">
	...
	<div style="border:1px solid #999;padding:3px;background:#ADD8E6;">
		<h5>
			<fmt:message key="stats.school.title1" />
		</h5>
		...
		<table id="table" class="table table-striped table-bordered"
			...
			<thead>
				<tr>
					...
					<th name="news" width="100px"><fmt:message
							key="stats.school.news" />
					</th>
					<th name="notice" width="100px"><fmt:message
							key="stats.school.notice" />
					</th>
					<th name="weibo" width="100px"><fmt:message
							key="stats.school.weibo" />
					</th>
...
<%=school.getId()%>
	...
	// PIE CHART
						...
						graph.id = "g1";
						graph.title = "<fmt:message key='stats.school.news'/>";
						graph.valueField = "news";
						graph.bullet = "round";
						...
						graph1.title = "<fmt:message key='stats.school.notice'/>";
						graph1.valueField = "notice";
						graph1.bullet = "round";
						...
						graph2.title = "<fmt:message key='stats.school.weibo'/>";
						graph2.valueField = "weibo";
						graph2.bullet = "round";
	...
	function select() {
		...
		restSelect(url, {
			...
			stars: null
			uploads: null */
			pieData[0].name = "<fmt:message key='stats.school.news'/>";
			pieData[1].name = "<fmt:message key='stats.school.notice'/>";
			pieData[2].name = "<fmt:message key='stats.school.weibo'/>";
			for ( var i = 0; i < 10; i++, j--) {
				jData[i] = logData[j];
				if (jData[i].news == null)
					jData[i].news = 0;
				if (jData[i].notice == null)
					jData[i].notice = 0;
				if (jData[i].weibo == null)
					jData[i].weibo = 0;
				pieData[0].visits += jData[i].news;
				pieData[1].visits += jData[i].notice;
				pieData[2].visits += jData[i].weibo;
			}
			for ( var i = 0; i < logData.length; i++) {
				count += logData[i].news;
				count += logData[i].notice;
				count += logData[i].weibo;
			}
			spinner.stop();//加载进度条关闭
			draw();
			...
			$("#count").text("时间段内总发送条目数为" + count + "条");
